"""OpenAI / GPT implementation of the Neural Engine."""
...
try:
    from openai import APIError, APIConnectionError, APIStatusError, RateLimitError  # type: ignore
...
class OpenAINeuralEngine(BaseNeuralEngine):
    """OpenAI-backed trading analyst. Uses native JSON mode where supported."""
    ...
    def _call(self, system: str, user: str, *, json_mode: bool) -> Dict[str, Any]:
        start = time.time()
        ...
        response = call_with_retry(_do, retries=3, base_delay=1.0, retryable_exceptions=_RETRYABLE)
        ...
        tokens_used = getattr(usage, "total_tokens", input_tokens + output_tokens) if usage else 0
        return {
    ...
    def analyze_trade(
        ...
        try:
            ...
            return self._error_analysis(f"OpenAI API error: {e}", raw="", tokens=0, latency=0.0)
    ...
    def analyze_portfolio(self, positions: List[Dict], market_overview: Dict) -> Dict:
        ...
        try:
            ...
            logger.error("OpenAI portfolio review invalid JSON: %s", e)
    ...
    def optimize_strategy(self, backtest_results: Dict, current_params: Dict) -> Dict:
        ...
        try:
            ...
            raise ValueError(f"OpenAI strategy optimization returned invalid JSON: {e}")
