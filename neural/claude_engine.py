...
class ClaudeNeuralEngine(BaseNeuralEngine):
    ...
    def _invoke_json(self, user_prompt: str) -> Dict[str, Any]:
        ...
        message = retry_with_backoff(lambda: self.queue.run(_call))
        raw_response = message.content[0].text
        parsed = json.loads(raw_response.strip().removeprefix("```json").removesuffix("```").strip())
    ...
    def analyze_portfolio(self, positions: List[Dict[str, Any]], market_overview: Dict[str, Any]) -> Dict[str, Any]:
        try:
            ...
            logger.error("Claude portfolio analysis failed: %s", exc)
    ...
    def explain_trade(self, trade_record: Dict[str, Any]) -> str:
        try:
            ...
            logger.error("Claude trade explanation failed: %s", exc)
    ...
    def generate_market_brief(self, watchlist: List[str], market_data: Dict[str, Any]) -> str:
        try:
            ...
            logger.error("Claude market brief failed: %s", exc)
